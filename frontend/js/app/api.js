  ...
  return new Promise(function (resolve, reject) {
    ...
    if (
      (typeof options.contentType === "undefined" ||
        options.contentType.match(/json/im)) &&
      typeof data === "object"
    ) {
    ...
    $.ajax({
        ...
        if (total !== null) {
          resolve({
            ...
            pagination: {
              ...
              offset: parseInt(
                response.getResponseHeader("X-Dataset-Offset"),
                10
              ),
              limit: parseInt(
                response.getResponseHeader("X-Dataset-Limit"),
                10
              ),
            },
          });
      ...
      error: function (xhr, status, error_thrown) {
        ...
        if (
          ...
          typeof xhr.responseJSON.error.message !== "undefined"
        ) {
      ...
      if (this.readyState === XMLHttpRequest.DONE) {
        if (xhr.status !== 200 && xhr.status !== 201) {
          try {
            reject(
              new Error(
                "Upload failed: " + JSON.parse(xhr.responseText).error.message
              )
            );
          } catch (err) {
...
module.exports = {
  ...
  Tokens: {
     ...
     */
    refresh: function () {
      return fetch("get", "tokens").then((response) => {
        if (response.token) {
  ...
  Users: {
    ...
    getById: function (user_id, expand) {
      return fetch(
        ...
        "users/" +
          user_id +
          (typeof expand === "object" && expand.length
            ? "?expand=" + makeExpansionString(expand)
            : "")
      );
    },
  ...
  Nginx: {
    ...
    RedirectionHosts: {
      ...
      setCerts: function (id, form_data) {
        return FileUpload(
          "nginx/redirection-hosts/" + id + "/certificates",
          form_data
        );
      },
    ...
    Certificates: {
      ...
      create: function (data) {
        const timeout =
          180000 +
          (data && data.meta && data.meta.propagation_seconds
            ? Number(data.meta.propagation_seconds) * 1000
            : 0);
      ...
      renew: function (id, timeout = 180000) {
        return fetch("post", "nginx/certificates/" + id + "/renew", undefined, {
          timeout,
        });
      },
